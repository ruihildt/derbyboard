...
export type BrandingSize = 'hidden' | 'small' | 'medium' | 'large';

/** Logo width as a fraction of the canvas/region width, per selected size. */
export const BRANDING_FRACTIONS: Record<BrandingSize, number> = {
	hidden: 0,
	small: 0.12,
	medium: 0.2,
	large: 0.3
...
export const BRANDING_PAD_FRACTION = 0.03;

/**
 * Loads the derbyboard logo once and stamps it as a screen-fixed branding
 ...
 * width, so the branding looks identical (relatively) on a phone or a large
...
export class Branding {
	...
	/**
	 * Draw the branding into the bottom-right corner of the given context, in
	...
	draw(
		...
		width: number,
		height: number,
		size: BrandingSize = 'medium'
	): void {
		if (!this.loaded || !this.image.complete) return;

		const frac = BRANDING_FRACTIONS[size];
		if (frac <= 0) return;
		...
		const padding = width * BRANDING_PAD_FRACTION;
